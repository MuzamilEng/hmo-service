...
import { useGetDataQuery, useGetEducationQuery, useGetJambfeeQuery } from "../pages/store/vtpassApi";

let SlideToggle;

const Education = () => {
  const {data} = useGetEducationQuery();
  const {data : userData} = useGetJambfeeQuery();
  ...
  const initialToggleState = state?.map(() => false);
  ...
  const initialToggleStates = state1?.map(() => false);
          ...
          <div className="col-xl-5 d-flex">
            <div className="card flex-fill">
              <div className="card-header">
                <h5 className="card-title">Education Related Payments</h5>
              </div>
              <div className="card-body">
                <div className="row">
                  ...
                  <div className="form-group row">
                    ...
                    <div className="col-lg-7">
                      <select className="form-control" onChange={changeForm}>
                        ...
                        <option value="2">JAMB Registration </option>
                        <option value="3">JAMB PIN</option>
                        <option value="4">NABTEB Registration</option>
                        <option value="5">NABTEB Result PIN</option>
                      </select>
                    </div>
                  </div>
              ...
              <div className="card-body">
                ...
                <div className="table-responsive" style={{ marginLeft: "-1rem" }}>
                  ...
                  <div className="" style={{ marginLeft: "-5rem" }}>
                    ...
                    {state?.slice(-3)?.map((item, index) => {
                      ...
                      return (
                        <div key={index} className="flex p-1 mt-2 justify-evenly border-b-2 border-gray-900">
                         ...
                         <p style={{width: "4.3rem"}}>{item?.responseData?.content?.transactions?.type}</p>
                          <p style={{fontSize: ".9rem"}}>{item?.responseData?.content?.transactions?.unique_element}</p>
                          <p style={{width: "4rem"}}>{item?.responseData?.amount}</p>
                    ...
                    {state1?.slice(-3)?.map((item, index)=> {
                      {console.log(item, "console item2jenu");}
                     return (
                       <div key={index} className="flex p-1 mt-2 justify-evenly border-b-2 border-gray-900">
                            <p>{name1?.username}</p>
                        <p style={{width: "4.2rem", paddingLeft: '.8rem'}}>{item?.responseData?.content?.transactions?.product_name}</p>
                        <p style={{fontSize: ".9rem", marginLeft: "1rem"}}>{item?.responseData?.content?.transactions?.unique_element}</p>
                        <p style={{width: "5rem"}}>{item?.responseData?.amount}</p>
                        <div className="relative">
                            <p
                              className="text-2xl cursor-pointer"
