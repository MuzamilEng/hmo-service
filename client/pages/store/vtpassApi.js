...
export const vtpassApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getDataservice: builder.query({
      query: () => `/data`,
    }),
    getElectricity: builder.query({
      query: () => `/electricity`,
      invalidatesTags: ['Eletricity']
    }),
    ...
    addElectricity: builder.mutation({
      ...
      async onQueryStarted(data, {dispatch, queryFullfilled}) {
        const Pdata = await queryFullfilled;
        console.log(Pdata, 'Pdata');
      },

      providesTags: ['Eletricity'],
    }),
...
export const
  { useGetDataQuery, useGetElectricityQuery,useAddElectricityMutation,
    useAddAirTimeMutation,useGetTvcableQuery, useAddDataMutation,
    useGetEducationQuery,useGetJambfeeQuery, useGetDataserviceQuery,
