...
import { estateBill, estateServices, guestInvited,} from '../Onboarding/Data';
...
import Card3 from '../../../_components/State/Card3';
          ...
          <div className="flex"> 
            ...
            <div className="absolute w-full max-w-[88rem] right-1">
              ...
              <div className="container pl-[2rem]">
                {estateBill?.map((item, index) => (
                  <div key={index} className='mb-2' >
                      <h3 className='text-xl font-bold'>{item.title}</h3>
                    <div className="pt-3">
                        {item?.form?.map((item2, index) => (
                          <div key={index} className='bg-[#fff]'>
                            <Card3 icon={item2?.icon} title={item2?.title} paid={item2?.paid} unpaid={item2?.unpaid} icon2={item2?.icon2} charges={item2?.charges}  price={item2?.price} tenant={item2?.tenant} bgColor={item2?.bg} time={item2?.time}/>
                          </div>
                        ))}
                    </div>
            ...
            <div className="flex justify-center items-center mt-4 ml-5 pb-3">
                  <Link href='/StateManagement/Statemgt/Paylist'>
