import React from 'react'
import Header from '../../../layouts/Header';
import Navbar from '../../../_components/State/Header';
import Sidebar from "../../../layouts/Sidebar";
import DropdownList from "../../../src/Compoment/DropdownList"
import Invoices from '../../../_components/State/Invoices';
import Link from 'next/link';
import { Fees, Orders } from '../Onboarding/Data';

const Order = () => {
  let arr = [];
  return (
    <>
      <div>
        <div className="">
          <Header />
          <div className="flex">
            <div className="mt-[2.6%] fixed pt-3 z-10">
              {/* <DropdownList /> */}
              <Sidebar />
            </div>
            <div className="absolute w-full max-w-[88rem] right-1">
              <Navbar />
              <div className="flex ">
                <div className="content mt-2 border-1 bg-[#ffff] border-gray-400 rounded-xl p-2 w-full ml-3">
                  {Orders?.map((item, index) => {
                    return <div key={index}>
                      <div className="flex justify-center mb-2 pb-2 m-2 items-center text-2xl bg-gray-400 font-semibold p-2 text-black"><h1>{item?.title}</h1></div>
                      <div className="flex justify-center items-center">
                        <span style={{fontSize: "20rem"}}>{item?.icon}</span>
                      </div>
                      <div className="flex flex-col">
                        {item?.form?.map((item2, index) => (
                          <div className="flex flex-col items-center justify-center" key={index}>
                            <h1 className='text-black text-xl  m-2 font-bold'>{item2?.info1}</h1>
                            <p className='text-black text-lg max-w-[50rem] text-justify'>{item2?.info}</p>
                            <p className='text-lg'>Order Number: {item2?.order_number}</p>
                            <span className='text-lg'>Order Date: {item2?.date}</span>
                            <span className='text-lg text-black'>Person Name: {item2?.details}</span>
                            <span>{item2?.pr_address}</span>
                          </div>
                        ))}
                    </div>
                    </div>
                  })}
                <div className="flex justify-around items-center mt-4 pt-4 ml-5 pb-3">
                  <Link href='/StateManagement/Onboarding/BranchOffice'>
                  <button className='text-white text-base bg-gray-500 p-2 max-w-[15rem] w-full rounded-full'>Join Another Community</button>
                  </Link>
                  <button className='text-white text-base bg-black mr-5 p-2 w-[10rem] rounded-full'>Close</button>
                </div>
              </div>
              {/* 2nd form */}
              {/* <div className="content mt-2 border-1 bg-[#ffff] border-gray-400 rounded-xl p-2 w-full max-w-[40rem] ml-3">
                  {Fees?.map((item, index) => {
                    return <div className='' key={index}>
                      <div className="flex mb-2 pb-2 text-2xl underline text-black"><h1>{item?.title}</h1></div>
                      <p>{item?.info}</p>
                      <div className="flex flex-col">
                        {item?.form?.map((item2, index) => (
                          <div key={index} className='flex h-[4rem] border-b-2 border-gray-400'>
                            <div className="flex items-center m-1 w-[30rem]">
                              <input className='w-5 h-5 shadow-green-600' type={item2?.type} />
                              <p className='ml-2 text-base'>{item2?.title}</p>
                            </div>
                            <div className="flex m-2 items-center">
                              <select className='text-lg w-[5rem] text-black p-1.5 border-2 rounded-lg m-2 bg-[#fbfcff]  focus:outline-none'>
                                {item2.month.map((option, index) => (
                                  <option key={index} value={option.value} className='focus:shadow-md'>
                                    {option.title}
                                  </option>
                                ))}
                              </select>
                              <input type={item2?.amount} placeholder={item2?.placeholder} className='border-2 rounded-md p-1 w-[5rem] border-gray-400 m-2' />
                            </div>
                          </div>
                        ))}
                      </div>
                    </div>
                  })}
                  <div className="flex justify-center items-center mt-4 ml-5 pb-3">
                  <Link href='/StateManagement/Pricing/Order'>
                  <button className='text-white text-xl bg-black p-1 w-[10rem] rounded-lg'>Create Fee</button>
                  </Link>
                </div>
                </div> */}
            </div>
          </div>
        </div>
      </div>
    </div >
    </>
  )
}

export default Order