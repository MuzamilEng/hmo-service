...
import { maintainerForm } from './Data'
import { Controller, useForm } from 'react-hook-form';
import { useRouter } from 'next/router';

const MaintainerForm = ({handleButtonClick, progress}) => {
    ...
    <div>
      <div className=" max-w-[28rem] w-full">
              <div className="content mt-2 border-1 bg-[#ffff] border-gray-400 rounded-xl p-2 ml-3">
                <form onSubmit={handleSubmit(onSubmit2)} className='max-w-[26rem] w-full'>
                {maintainerForm?.map((item, index) => {
              ...
              <div className="flex justify-center items-center mt-4 ml-5 pb-3">
                <button className='text-white text-xl bg-black mt-[17rem] p-1 w-[10rem] rounded-lg' onClick={handleButtonClick} value={progress}>Continue</button>
              </div>
              </form>
            </div>
              </div>
...
// import { Location, maintainerForm } from './Data';
...
//                                 type={item2?.type}
//                                 placeholder={item2?.placeholder}
//                                 className={`w-full max-w-[30rem] text-black p-2.5 border-[1.5px] rounded-lg m-2 focus:border-blue-900 focus:outline-blue-900 ${item2?.type === 'date' && 'custom-input-date appearance-none leading-tight'}`}
//                               />
//                             )}
//                           </div>
//                         ))}
...
//                   })}
//                 <div className="flex justify-center items-center mt-4 ml-5 pb-3">
//                   <Link href='/StateManagement/Onboarding/MaintiReq'>
